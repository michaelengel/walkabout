...
char *mem;
void (*specialised_emu_params)(char *);

void default_empty(char *) {
}
...
int main (int argc, char **argv, char **env)
{
    ...
    unsigned tracept = 0;
	specialised_emu_params = default_empty;
    
    while(1) {
        int c = getopt( argc, argv, "+dDp:s:" );
        if( c == -1 ) break;
        switch(c) {
            case 'd':
                debug = true;
    ...
    if( optind >= argc ) {
        ...
        fprintf( stderr, "    -s  Specialised emulator parameters\n" );
        exit(1);
    }
